  ...
  methods:{
    ...
    async fetchProducts(){
      ...
      console.log(this.$axios.defaults.baseURL)
        let p_ = await fetch(this.$axios.defaults.baseURL + `api/products`).then(res=>res.json()).then(data=>{return {ok:true, data:data}}).catch(error=>{return {ok:false,error:error}});
        console.log(p_);
        if(p_.ok){
